...
import type { TaxonProfilesFeature, FeatureBase } from "../types/Features";
...
import type{ ImageWithTextAndLicence } from '../types/Image';
import type { MatrixFilterDataTypes } from './NatureGuide';
import type { GenericFormReference } from './GenericForm';
...
export class TaxonProfiles {
  ...
  async loadLocalizedRegistry (languageCode: string) {
    const url: string = this.taxonProfilesFeature.localizedRegistries[languageCode];
    const response = await fetch(url);
    if (response.ok) {
